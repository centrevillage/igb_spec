...
選択可能な通信レートは 4Mbps、2Mpbs、1Mbps、400kbps、115200bps、31250bps の６つであるが、1Mbps以外のレートについては全て実装する必要はない。\
...
```
0xF4, 0x60, バルク番号(14bit), デバイスチャネル番号, パラメータ番号, 更新レート(7bit, 100Hz単位), パラメータデータ1-128, 0xF7
```

14bit以上のデータの場合は上位桁から順に7bit送信するものとする。\
バルク番号は処理中のバルクデータと重複しないよう連番で採番することが望ましい。\
更新レートは1-120までの値が設定可能であり、100Hzから12kHzまで利用できる。\
