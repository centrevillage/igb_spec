...
- パラメータセットへの登録(`PARAM` push + `PRMSET`)
...
フレーズごとに、各トラックのパターンの時間配置とパラメータセットの時間配置を行う。\
`PARAM`ノブで編集カーソルの移動、編集トラックの移動と対象フレーズの移動が可能。
...
- フレーズの繰り返し数設定
- フレーズのループスタートポイント設定
- フレーズからのパターン/パラメータセット削除
- パターン/パラメータセットの配置位置の移動
- 配置済みパターン/パラメータセットの変更

情報表示項目:
- 配置されたパターンの番号とその位置と長さ
- 配置されたパラメータセットの番号と名前とその位置
- 編集対象のフレーズ番号
...
1トラックあたり16個のパラメータセットを扱うことができる。\
パラメータセットの読み込みは `Step Key` で行うことができる、

### 4-x. パラメータセット編集

モードへの遷移: `EDIT + PRMSET` or パラメータセット選択で `EDIT + Step Key`

各パラメータセットに登録されたパラメータの編集ができる。\
この画面でパラメータの登録削除も可能。\
ここで選択したパラメータセットが編集対象のパラメータセットとして扱われる。\
(パラメータ編集画面でのパラメータ登録対象になる)

設定項目:
- パラメータセットへのパラメータ登録/変更/削除
- 編集対象パラメータセットの変更

情報表示項目:
- パラメータセットに登録されたパラメータの名前と値

### 4-x. パターン管理

モードへの遷移: `EDIT + PATTERN`

現在クイックアクセス可能なパターンの一覧と、パターンの入れ替えを行うことができる。

設定項目:
- 各 `PATTERN` ボタンに割り当てるパターンの変更
- 各パターンの切り替えタイミング(即時 or パターンend)

情報表示項目:
- トラックの全パターンの一覧
- トラックのクイックアクセスパターンの一覧

### 4-x. シーケンス設定

モードへの遷移: `EDIT + SEQ`
 
最大16のシーケンスに対してのパラメータの割り当てを行うことができる。
...
編集対象シーケンスの変更とシーケンスの入力を行うことができる。\
1画面で4つのシーケンスパラメータを表示＆編集可能。\
シーケンスはページあたり最大16ステップ入力可能で、さらに16ページまで利用できる。\
シーケンス編集モード中に `EDIT` を押すことで、表示ページ位置を現在ステップに同期させるか切り替え可能。

設定項目:
- タイミングディレイ(`FUNC` + `PARAM`)
- ゲートレングス(`FUNC` + `EDIT` + `PARAM`)
- 各ステップへのトリガー入力(`PARAM` push -> `Step Key`)
- 各ステップへのパラメータ入力(`PARAM`)
- シーケンスパラメータページ切り替え(`PREV` or `NEXT`)
- シーケンスページ切り替え(`FUNC` + `Step Key`)

### 4-x. マイクロステップ編集 

モードへの遷移: シーケンス編集で `EDIT + Step Key`

編集ステップに対して、マイクロステップの設定を行うことができる。

設定項目:
- マイクロステップのトリガー入力
- マイクロステップのゲートレングス(マイクロステップ全体)
- マイクロステップのパラメータ入力
- マイクロステップレングス/分解能
...
`A` ~ `Z` までの26のマクロパラメータを設定可能。\
パラメータごとに最大4つのセンドパラメータを設定可能。\
センド量には別のマクロパラメータを設定することが可能で、その場合は二つのマクロパラメータの積が最終的なモジュレーション量となる。
...
各パフォーマンスノブにパラメータもしくはマクロパラメータを割り当てる。
...
各リボンモードにパラメータもしくはマクロパラメータを割り当てる。
...
キーボードでのリアルタイム値反映の対象とするパラメータを設定する。\
シーケンス編集モードではキーボードはパラメータ入力として扱われるので、リアルタイム反映は自動的に無効化される。
...
モードへの遷移: `FUNC + KEYBOARD`

12のスケールプリセットの表示および現在のスケールの変更を行う。
...
ミキサーの各トラックの音声入力割り当てを行う。\
また `MUTE` ボタンの動作割り当てを行う。

設定項目:
- 音声入力元
- `MUTE` ボタンの動作(オーディオミュート/トラックミュート)
